...
export default class DataModelSourceDiffImpl {
    constructor(public git, public baseline: string, public target: string, public packageDirectories: string[]) {}
    ...
    public async exec(): Promise<any> {
        ...
        for (let metadataType in this.diffGenerators) {
            ...
            if (!isNullOrUndefined(this.packageDirectories)) {
                changedFiles = this.filterByPackageDirectory(changedFiles, this.packageDirectories);
            }

            let sourceDiffGenerator: DiffGenerators.SourceDiffGenerator = new this.diffGenerators[metadataType](
                this.baseline,
                this.target
            );

            for (let file of changedFiles) {
                let fileRevFrom: string | void = await this.git.show([`${this.baseline}:${file}`]).catch((err) => {});

                let fileRevTo: string | void = await this.git.show([`${this.target}:${file}`]).catch((err) => {});

                let diff = await sourceDiffGenerator.compareRevisions(fileRevFrom, fileRevTo, file);
    ...
    private filterByPackageDirectory(changedFiles: string[], packageDirectories: string[]): string[] {
