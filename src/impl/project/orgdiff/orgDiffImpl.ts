...
const unsplitedMetadataExtensions = UNSPLITED_METADATA.filter((elem) => {
    return elem.xmlName !== METADATA_INFO.Profile.xmlName && elem.xmlName !== METADATA_INFO.PermissionSet.xmlName;
}).map((elem) => {
...
export default class OrgDiffImpl {
    private output = [];
    public constructor(private filesOrFolders: string[], private org: Org, private addConflictMarkers: boolean) {}
    ...
    private compare() {
        ...
        let fetchedFiles = FileUtils.getAllFilesSync(`./temp_sfpowerkit/source`, '');
    ...
    private processFile(localFile: string, fetchedFiles: string[]) {
        ...
        let foundFile = fetchedFiles.find((fetchFile) => {
            ...
            let fetchedMember = MetadataFiles.getMemberNameFromFilepath(fetchFile, fetchedMetaType);
            //let fetchedExtension = path.parse(fetchFile).ext;
            let fetchedCmpPath = path.parse(fetchFile).base;

            let fetchedMetadataDescribe: MetadataDescribe = METADATA_INFO[fetchedMetaType];
            if (fetchedMetadataDescribe.inFolder || staticResourceRegExp.test(fetchFile)) {
                ...
                let fetchedBaseIndex = fetchFile.indexOf(fetchedFolderName) + fetchedFolderName.length;
                fetchedCmpPath = fetchFile.substring(fetchedBaseIndex + 1);
            }
            return fetchedMetaType === metaType && member === fetchedMember && cmpPath === fetchedCmpPath;
        });
        ...
        if (foundFile !== undefined) {
            ...
            if (!contentLocalFile.endsWith(lineEnd) && contentFetchedFile.endsWith(lineEnd)) {
                contentFetchedFile = contentFetchedFile.substr(0, contentFetchedFile.lastIndexOf(lineEnd));
            }
        ...
        for (let i = 0; i < diffResult.length; i++) {
            ...
            if (result.removed) {
                if (firstChunkProcessed) {
                    ...
                    (originalArray.length > index + 1 && originalArray[index + 1].added == undefined) ||
                    ...
                    content = content + `<<<<<<< Local:${filePath}\n${result.value}=======\n`;
    ...
    private async retrievePackage(packageObj) {
        ...
        child_process.execSync(sfdxConvertCommand, {
            encoding: 'utf8',
            stdio: 'inherit',
