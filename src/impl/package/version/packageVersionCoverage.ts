...
const DEFAULT_ORDER_BY_FIELDS = 'Package2Id, MajorVersion, MinorVersion, PatchVersion, BuildNumber';
export default class PackageVersionCoverage {
    ...
    public async getCoverage(
        ...
        let whereClause = (await this.getWhereClause(versionId, versionNumber, packageName)) as string;

        if (!whereClause) {
            throw new Error('Either versionId or versionNumber and packageName is mandatory');
        }
        ...
        if (result && result.size > 0) {
            result.records.forEach((record) => {
                ...
                packageCoverage.HasPassedCodeCoverageCheck = record.HasPassedCodeCoverageCheck;
                packageCoverage.coverage = record.CodeCoverage ? record.CodeCoverage.apexCodeCoveragePercentage : 0;
    ...
    private async getWhereClause(versionId: string[], versionNumber: string, packageName: string): Promise<string> {
        let whereClause = '';
        if (versionId && versionId.length > 0) {
            whereClause = this.buildWhereFilter('SubscriberPackageVersionId', versionId);
        } else if (versionNumber && packageName) {
            whereClause =
                this.buildWhereOnNameOrId('0Ho', 'Package2Id', 'Package2.Name', packageName) +
                ' AND ' +
    ...
    private buildWhereOnNameOrId(idFilter: string, idKey: string, nameKey: string, value: string) {
        let result = '';
