...
export default function writeProfile(profileObj: Profile, filePath: string) {
...
export function toProfile(profileObj: any): Profile {
    ...
    if (Array.isArray(profileObj[key])) {
      ...
      if (nonArayProperties.includes(key)) {
        ...
        for (var i = 0; i < profileObj[key].length; i++) {
          var element = removeArrayNatureOnValue(profileObj[key][i]);
          if (element !== "") {
            data.push(element);
          }
        }
...
function removeArrayNatureOnValue(obj: any): any {
  var toReturn = {};
  for (var key in obj) {
