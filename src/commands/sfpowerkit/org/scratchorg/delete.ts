...
export default class Delete extends SFPowerkitCommand {
    ...
    public async execute(): Promise<AnyJson> {
        if (!this.flags.username && !this.flags.email) {
            throw new SfdxError('Required flags are missing, Please provide either username or email.');
        }
        ...
        let info = await this.getActiveScratchOrgsForUser(conn, this.flags.email, this.flags.username);

        if (info.totalSize > 0) {
            this.ux.log(
                `Found ${info.totalSize} Scratch Org(s) for the given ${
                    this.flags.username ? 'Username: ' + this.flags.username : 'Email: ' + this.flags.email
                } in devhub ${this.hubOrg.getUsername()}.\n`
            );
            this.ux.table(info.records, ['Id', 'ScratchOrg', 'SignupUsername', 'SignupEmail', 'ExpirationDate']);
    ...
    private async getActiveScratchOrgsForUser(conn: Connection, email: string, username: string): Promise<any> {
