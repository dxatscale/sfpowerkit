...
export default class Deactivate extends SfpowerkitCommand {
  ...
  `,
    ...
    public async execute(): Promise<AnyJson> {
        ...
        if (!metadata_retrieve_result.zipFile) throw new SfdxError('Unable to find the requested Trigger');
        ...
        if (fs.existsSync(path.resolve(resultFile))) {
            ...
            let xml = builder.buildObject(retrieve_apextrigger);
            ...
            this.ux.log(`Deploying Deactivated ApexTrigger with ID  ${deployId.id} to ${this.org.getUsername()}`);
