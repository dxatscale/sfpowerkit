...
const messages = Messages.loadMessages('sfpowerkit', 'matchingrule_deactivate');
...
export default class Deactivate extends SFPowerkitCommand {
  ...
  `,
    ...
    public async execute(): Promise<AnyJson> {
        ...
        if (!metadata_retrieve_result.zipFile) throw new SfdxError('Unable to find the requested Matching Rule');
        ...
        if (fs.existsSync(path.resolve(resultFile))) {
            ...
            if (isJsonArray(retrieve_matchingRule.MatchingRules.matchingRules)) {
                ...
                if (!util.isNullOrUndefined(retrieve_matchingRule.MatchingRules.matchingRules))
            ...
            let xml = builder.buildObject(retrieve_matchingRule);
            ...
            this.ux.log(`Deploying Deactivated Matching Rule with ID  ${deployId.id}  to ${this.org.getUsername()}`);
