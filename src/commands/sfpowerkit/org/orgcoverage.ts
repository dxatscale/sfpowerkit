...
export default class OrgCoverage extends SFPowerkitCommand {
  ...
  `,
    ...
    public async execute(): Promise<AnyJson> {
        ...
        this.ux.log(`Successfully Retrieved the Apex Test Coverage of the org ${this.org.getOrgId()} `);
        this.ux.log(`coverage:${apexcoverage.coverage}`);

        const classCoverage = await this.getApexCoverageByDetails(conn, this.flags.output);

        return { coverage: apexcoverage.coverage, classCoverage: classCoverage };
    }

    private async getApexCoverage(conn: Connection) {
        let encoded_querystring = querystring.escape(`SELECT PercentCovered FROM ApexOrgWideCoverage`);
    ...
    private async getApexCoverageByDetails(conn: Connection, outputDir: string) {
        ...
        if (results.size > 0) {
            results.records.forEach((element) => {
                ...
                if (element.NumLinesCovered === 0 && element.NumLinesUncovered === 0) {
                    ...
                    percentage = this.percentCalculate(element.NumLinesCovered, element.NumLinesUncovered);
            ...
            this.ux.table(output, ['id', 'package', 'name', 'type', 'percentage', 'comments', 'uncoveredLines']);
    ...
    private percentCalculate(covered: number, uncovered: number) {
        return covered === 0 ? 0 : Math.round((covered / (covered + uncovered)) * 100);
    }
    ...
    private async generateCSVOutput(testResult: any[], outputDir: string) {
        ...
        let newLine = '\r\n';
        let output = 'ID,PACKAGE,NAME,TYPE,PERCENTAGE,COMMENTS,UNCOVERED LINES' + newLine;
    ...
    private async getmetadataVsPackageMap(conn: Connection) {
        ...
        let packageMember: Map<string, string> = await DependencyImpl.getMemberVsPackageNameMapByKeyPrefix(
            conn,
        ...
        for (let subjectId of metadataMap.keys()) {
            if (packageMember.has(subjectId)) {
                metadataVsPackageMap.set(metadataMap.get(subjectId).fullName, packageMember.get(subjectId));
            }
        }
