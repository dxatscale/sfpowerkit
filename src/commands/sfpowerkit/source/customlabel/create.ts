...
const messages = Messages.loadMessages('sfpowerkit', 'source_customlabel_create');
...
<Package xmlns="http://soap.sforce.com/2006/04/metadata">
    ...
    <version>${this.flags.apiversion}</version>
        ...
        this.ux.log(`Deploying Custom Label with ID  ${deployId.id} to ${this.org.getUsername()}`);
        ...
        if (metadata_deploy_result.success) {
            if (!this.flags.ignorepackage)
                ...
                this.ux.log(`Deployed  Custom Label ${this.customlabel_fullname} in target org`);
        } else {
            throw new SfdxError(
