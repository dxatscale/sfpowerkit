...
export default class Convert extends SfdxCommand {
  ...
  protected static flagsConfig = {
    ...
    pathoverride: flags.string({ required: false, char: 'o', description: messages.getMessage('pathFlagDescription') })
  ...
  public async run(): Promise<AnyJson> {
    ...
    if (fs.existsSync(path.resolve(apextestsuite_file_path))) {
      ...
      let apex_test_suite = await parseString(fs.readFileSync(path.resolve(apextestsuite_file_path)));
      
      let apex_test_suite_as_string= JSON.stringify(apex_test_suite.ApexTestSuite.testClassName);

      apex_test_suite_as_string = apex_test_suite_as_string.replace(/("|')/g, "");
