...
const messages = Messages.loadMessages('sfpowerkit', 'dependency_versionlist');
...
export default class List extends SFPowerkitCommand {
    ...
    protected static flagsConfig = {
        ...
        usedependencyvalidatedpackages: flags.boolean({
            required: false,
            description: messages.getMessage('usedependencyvalidatedpackagesDescription'),
        }),
    };
    ...
    public async execute(): Promise<any> {
        ...
        for (let packageDirectory of projectConfig.packageDirectories) {
            if (filterpaths.length == 0 || filterpaths.includes(packageDirectory.path)) {
                if (packageDirectory.dependencies && packageDirectory.dependencies[0] !== undefined) {
                    this.ux.log(`Package dependencies for the given package directory ${packageDirectory.path}`);
                    for (let dependency of packageDirectory.dependencies) {
                        if (projectConfig.packageAliases[dependency.package]) {
                            await this.getPackageVersionDetails(conn, dependency, projectConfig.packageAliases);

                            this.ux.log(
                                `    ${dependency.versionId} : ${dependency.package}${
                                    dependency.versionNumber === undefined ? '' : ' ' + dependency.versionNumber
                                }`
                            );
                        } else {
                            this.ux.warn(
    ...
    private async getPackageVersionDetails(conn: Connection, dependency: any, packageAliases: any) {
        ...
        if (packageId.startsWith(packageVersionIdPrefix)) {
            ...
            if (!dependency.versionNumber) {
                throw new SfdxError(`version number is mandatory for ${dependency.package}`);
            }
            ...
            if (resultPackageId.size === 0) {
                ...
                const errorMessage = `Unable to find package ${dependency.package} of version ${
