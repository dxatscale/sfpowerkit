...
export default class Install extends SfpowerkitCommand {
    ...
    public async execute(): Promise<any> {
        ...
        try {
            ...
            SFPLogger.log('Unable to retrieve the packages installed in the org, Proceeding', LoggerLevel.WARN);
        }
        ...
        if (this.flags.tag) {
            this.tagMap = this.parseKeyValueMapfromString(this.flags.tag, 'Tag', 'core:tag consumer:tag packageN:tag');
        }
                ...
                for (const dependency of dependencies as JsonArray) {
                    ...
                    if (individualpackage) {
                        if (
                            ...
                            packagesToInstall.set(packageInfo.packageVersionId.toString(), packageInfo);
                        }
                    } else if (this.flags.updateall) {
                        packagesToInstall.set(packageInfo.packageVersionId.toString(), packageInfo);
                    } else if (!installedpackages.includes(packageInfo.packageVersionId)) {
                        packagesToInstall.set(packageInfo.packageVersionId.toString(), packageInfo);
                    }
        ...
        if (packagesToInstall.size > 0) {
            ...
            this.ux.table(packagesToInstallArray, ['packageName', 'versionNumber', 'packageVersionId']);

            this.ux.log(`\n`);

            for (let packageInfo of packagesToInstallArray) {
                packageInfo = packageInfo as JsonMap;
                if (result.installedPackages.hasOwnProperty.call(packageInfo.packageVersionId.toString())) {
                ...
                // INSTALLATION KEY
                if (installationKeyMap && installationKeyMap.has(packageInfo.packageName.toString())) {
                ...
                this.ux.log(
                    `Installing package ${packageInfo.packageVersionId} : ${packageInfo.packageName}${
                        packageInfo.versionNumber === undefined ? '' : ' ' + packageInfo.versionNumber
                    }`
                );
                ...
                result.installedPackages[packageInfo.packageVersionId.toString()] = packageInfo;
            }
        } else {
    ...
    private async getPackageVersionDetails(name, version): Promise<{ versionId; versionNumber }> {
    ...
    private async getInstalledPackages(targetOrg: string) {
        ...
        return await retry(
            async (bail) => {
                SFPLogger.log('QUERY:' + installedPackagesQuery, LoggerLevel.TRACE);
    ...
    private parseKeyValueMapfromString(request: string, item: string, format: string) {
        ...
        for (let element of requestList) {
            ...
            if (packageNameWithValue.length === 2) {
                ...
                throw new SfdxError(`Error in parsing ${item}, format should be: ${format}`);
            }
        }
