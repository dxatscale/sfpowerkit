...
export default class Login extends SFPowerkitCommand {
  ...
  `,
    ...
    public async execute(): Promise<AnyJson> {
        ...
        if (this.flags.securitytoken) this.password = this.flags.password.concat(this.flags.securitytoken);
        ...
        await conn.login(this.flags.username, this.password, function (err) {
            if (err) {
